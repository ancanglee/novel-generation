      ...
      {q.isLoading ? <Spinner /> : (
        <DataTable<AlertRow>
          columns={[
            ...
            { key: "severity", header: "等级", render: (r) => <Badge tone={r.severity === "critical" ? "danger" : "warning"}>{r.severity}</Badge> },
            { key: "enabled", header: "启用", render: (r) => (r.enabled ? <Badge tone="success">ON</Badge> : <Badge>OFF</Badge>) },
          ]}
          rows={q.data?.alerts ?? []}
