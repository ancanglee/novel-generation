...
export default function ModelConfigsPage() {
  ...
  return (
    <div className="space-y-4 p-6">
          ...
          columns={[
            {
              ...
              header: "阶段",
              render: (r) => (
                <Badge>{admin.stages[r.stage as keyof typeof admin.stages] ?? r.stage}</Badge>
              ),
            },
            { key: "primary", header: "主模型", render: (r) => r.primary.model_id },
            {
              ...
              header: "参数",
              render: (r) =>
                `maxTokens=${r.primary.max_output_tokens} · temp=${r.primary.temperature}`,
            },
            ...
            { key: "version", header: "版本" },
            {
              key: "updated_by",
              header: "更新者",
              render: (r) => r.updated_by?.slice(0, 8) ?? "—",
            },
            {
              key: "actions",
              header: "操作",
      ...
      {editing ? <EditModelModal row={editing} onClose={() => setEditing(null)} /> : null}
    </div>
  );
}
